...
func main() {
	...
	if err := run(os.Args[1:], logger); err != nil {
		logger.Error("Synology authentication bridge stopped", "error", err)
		os.Exit(1)
	}
}
...
func run(arguments []string, logger *slog.Logger) error {
	...
	if err != nil || address.IP == nil || !address.IP.IsLoopback() {
		return errors.New("Synology authentication bridge must listen on an explicit loopback address")
	}
