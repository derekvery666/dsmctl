  ...
  <section class="card">
    ...
    <div class="body">
      ...
      <form method="post">
        ...
        {{if .DSMWebLogin}}<button class="button primary full" type="submit" name="auth_method" value="dsm">{{if .TraditionalChinese}}使用 DSM Web Login 授權{{else}}Authorize with DSM Web Login{{end}}</button>{{end}}
