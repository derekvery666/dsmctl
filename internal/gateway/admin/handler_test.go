...
func TestAdminUIHasNoEmbeddedCredential(t *testing.T) {
	...
	for _, required := range []string{
		...
		`.auth-shell{min-height:100vh;display:grid;grid-template-columns:minmax(420px,52%) minmax(420px,1fr);padding:0`,
		...
		`id="dsmLoginOption"`, `onclick="loginWithDSM()"`, `/dsm-login`, `/dsm-login/start`, `dsmctl_dsm_signin`, `dsmAdminLoginWaiting`, `callbackReceived=true`, `popup.closed&&!callbackReceived`, `window.addEventListener('message',listener);popup.location.href=start.login_url`, `id="localAdministratorDialog"`, `/local-administrator`,
