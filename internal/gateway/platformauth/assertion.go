// Package platformauth signs and verifies short-lived administrator assertions
// exchanged between a deployment adapter and the platform-neutral Gateway.
...
func (s *Signer) Sign(subject string) (string, error) {
	...
	claims := payload{
		Subject: subject, Administrator: true, Audience: s.audience,
		IssuedAt: now.Unix(), ExpiresAt: now.Add(s.ttl).Unix(), ID: hex.EncodeToString(idBytes),
	}
...
func (v *Verifier) Verify(ctx context.Context, assertion string) (Identity, error) {
	...
	if ValidateSubject(claims.Subject) != nil || !claims.Administrator || claims.Audience != v.audience || len(claims.ID) != 32 ||
		expiresAt.Sub(issuedAt) <= 0 || expiresAt.Sub(issuedAt) > MaxTTL || issuedAt.After(now.Add(clockSkew)) || !expiresAt.After(now) {
		return Identity{}, ErrUnauthorized
	}
	...
	if _, exists := v.seen[claims.ID]; exists || len(v.seen) >= maxReplayEntries {
		return Identity{}, ErrUnauthorized
	}
	v.seen[claims.ID] = expiresAt
	return Identity{Subject: strings.TrimSpace(claims.Subject)}, nil
}
