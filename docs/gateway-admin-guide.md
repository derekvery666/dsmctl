...
2. 建立 MCP Server 管理員。密碼至少 8 個字元，強度由使用者自行負責。
3. 第一次成功建帳前，設定頁會持續開放且重啟不會關閉；在完成前，請只讓
   loopback 或受信任部署網路存取 Gateway。
...
再重新完成第一次設定。所有 NAS Profile、Token 與 Audit
紀錄都會一併刪除，登入頁也有相同提示。
...
1. 前往「NAS 管理」並按「新增 NAS」。沒有任何 Profile 時，列表中央也會顯示
   「新增 NAS」；新增表單平常不會佔據頁面。
2. 精靈第一步會先顯示 IP、DNS 名稱或完整 DSM URL 的手動輸入欄位；僅在管理員
   按下「搜尋區域網路」後，才會從 MCP Server 所在的 broadcast domain 搜尋
   Synology 裝置，單純開啟精靈不會送出掃描。搜尋不跨路由器、VLAN 或 VPN；
   這些情況請直接手動輸入。
3. 搜尋完成後可用結果上方的欄位即時篩選名稱、型號、IP、DSM 版本、序號或 MAC；
   篩選只處理已取得的結果，不會再次掃描網路。每張結果卡依序顯示名稱與型號、
   DSM 版本與狀態、`S/N` 與主要 `MAC`，IP 則獨立靠右；若裝置回報多個 MAC，
   會以 `+N` 收斂其他介面。結果很多時只有結果清單會捲動，手動輸入、搜尋欄、
   「再次搜尋」與「取消」會留在原位。
4. 選取裝置或輸入位址後，第二步只要求確認容易辨識的 Profile 名稱與 container
   可連線的 DSM URL，例如 `https://192.168.1.20:5001`。從這個精靈建立的 NAS
   一律是受管 Profile；目的地專用角色仍保留給進階的非精靈設定。搜尋結果不保證
   知道自訂 DSM port，因此 URL 仍必須由管理員確認。連線使用內建 30 秒逾時；
   `0` 在儲存資料中代表採用此預設值，不是零秒，因此精靈不顯示逾時欄位。
5. 不需要預先選擇 TLS 模式或手動抄寫 fingerprint。系統一律先用 System CA、
...
6. Profile 名稱建立後不可更改。精靈第三步建議使用 `Web Login`；NAS 頁也永遠提供
   遮罩的「密碼/OTP」登入。若 Profile 已有主要密碼，列上的主要按鈕是「使用已存
   密碼連線」；若沒有，主要按鈕會開啟帳號、密碼與 OTP 表單。表單讓管理員明確勾選
   是否把密碼加密保存到該 NAS 的金庫：勾選時，主要密碼與這次取得的 DSM Session
   會一起保存；不勾選時只保留可重用的 DSM Session，密碼與新 trusted-device
   credential 不會寫入金庫。若已存 trusted device 過期或 DSM 要求 OTP，按鈕會指出
   原因並開啟同一表單供重新驗證。Web Login 一直保留在更多操作選單，兩種登入方式
   互不排斥。若先關閉精靈，列表會保留「完成設定」。
   「密碼」頁只管理金庫資料（新增、變更、顯示與移除帳密），不提供任何連線操作；
   額外保存的次要帳號也不會暗中取代 Profile 的執行身分。
7. 新增、編輯、完成設定與重新登入共用同一個三步驟精靈：新增從來源頁開始、
   編輯與完成設定從連線頁開始、重新登入從登入頁開始，且登入頁可返回檢查連線。
8. Profile 列表顯示 DSM 實際帳號與「已儲存」的驗證方式。這代表加密認證資料
   存在，不代表 NAS 此刻一定在線。編輯、連線診斷、重新驗證與刪除收在該列的
   更多操作選單；「連線診斷」會依序顯示 DNS、TCP、TLS/HTTP 與 DSM 驗證結果，
   不再以未說明的「測試」或原始 JSON 呈現。
9. 更多操作選單也提供「複製帳號」與「複製密碼」。「複製密碼」只在該 Profile
   ...
   機密，貼上後請自行清空。
10. NAS 頁的「匯出憑證」可一次匯出所有 Profile，套用同樣的人工把關。它要求
   管理員重新輸入自己的管理員密碼，通過後下載一個 CSV
   （`dsmctl-nas-credentials.csv`），每個 Profile 一列，欄位為 `name`、`host`、
   `url`、`account`、`password`；未儲存帳號或密碼者該欄留空。它只讀取 vault
   內已儲存的密碼（不含環境變數提供的密碼），依來源限制頻率，並記錄獨立的
   `credential.export` Audit 事件（只記錄 Profile 與已存密碼的數量，不含任何
   值）。密碼只出現在下載的檔案中，不會顯示在畫面上；檔案含明文密碼，請妥善
   保管並於使用後刪除。
