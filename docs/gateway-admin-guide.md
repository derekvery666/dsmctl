...
3. 選擇 TLS 驗證：**自簽憑證**使用預設的 `Pinned fingerprint`，並核對
   SHA-256 fingerprint；憑證更換（含 Let's Encrypt 自動續期）後必須更新
   指紋，否則連線會失敗。**已申請 CA 簽發憑證**（例如 Let's Encrypt）的
   NAS 請改用 `System CA`，憑證自動更新不受影響。
...
Session。變更管理密碼必須再次確認至少 8 個字元的新密碼，並會撤銷其他管理員
