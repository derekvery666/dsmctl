...
flowchart LR
  ...
  subgraph DP["Data protection (greenfield program)"]
    WI019 --> WI088["WI-088 Snapshot Replication"]
    WI022 --> WI088
  end
...
| [WI-088](work-items/WI-088-snapshot-replication.md) | P2 | `done` | C | WI-019, WI-022, WI-029 | Snapshot Replication module, **shipped + live-verified on DSM 7.3-81168**: per-share btrfs snapshot reads (attributes/config/retention/log/node — all core APIs, no package needed) + guarded snapshot create/edit/delete and share-config writes via hash-bound plan/apply (full lifecycle verified on a `dsmctl-e2e-snap-*` share); replication-plan read package-gated and fail-closed. Found + documented the DSM 7.3-81168 feed deadlock that makes the package uninstallable on that build (needs DSM ≥ 7.3-81179). Retention/schedule writes, replication mutations, restore/rollback, LUN snapshots deferred. WI-084–086 reserved by the provisioning program. |
