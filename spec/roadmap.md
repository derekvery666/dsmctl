...
flowchart LR
  ...
  WI032 --> WI091["WI-091 DSM-delegated Gateway login"]
  WI032 --> WI092["WI-092 Stored-password NAS connect"]
  WI023 --> WI093["WI-093 Add NAS discovery ergonomics"]
  WI047 --> WI093
  WI093 --> WI094["WI-094 Refine NAS discovery dialog layout"]
  WI094 --> WI095["WI-095 Simplify NAS wizard + SPK upgrade"]
  ...
  WI022 --> WI087["WI-087 Hyper Backup"]
  ...
  subgraph DP["Data protection (greenfield program)"]
    WI019 --> WI089["WI-089 Snapshot Replication"]
    WI022 --> WI089
    WI089 --> WI090["WI-090 Replication relation create"]
  end
...
| [WI-017](work-items/WI-017-amd64-linux-synology-distribution.md) | P1 | `in_progress` | G | WI-014, WI-015, WI-016, WI-032, WI-033, WI-035, WI-037, WI-038 | Ship downloadable CLI/MCP archives plus the same amd64 Gateway image for generic Linux and an offline Synology x86_64 Container Manager SPK; public releases remain preview until hardware/lifecycle certification completes. |
...
| [WI-032](work-items/WI-032-gateway-local-administrator.md) | P0 | `done` | G | WI-015, WI-016 | Replace bootstrap/platform administration with portable first-run local-account setup and browser sessions. |
...
| [WI-066](work-items/WI-066-firewall.md) | P1 | `done` | C | WI-006 | Firewall: **read + guarded-write slices shipped + live-verified on DSM 7.3**. Writes = full-profile `Firewall.Profile.set` (rule CRUD/reorder + default policy), `Profile.Apply.start` (enable/switch), `Firewall.set set_type=disable`; per-rule shape live-confirmed via a firewall-OFF round-trip (no `ip_version`). **Never-lockout guard** (`ProfileAllowsSession`, fail-closed, hashes connection+ruleset) proven both directions live plan-only; firewall never actually enabled on the shared lab (constrained verify). Remaining: built-in service-set→port resolution undecidable (fail-closed); profile lifecycle create/rename/clone out of scope. |
| [WI-067](work-items/WI-067-accountprotection.md) | P2 | `done` | C | WI-006 | Account protection: **read + guarded-write slices shipped + live-verified on DSM 7.3**. Writes = `AutoBlock.set` (binds only when enable=true), `AutoBlock.Rules` `create`/`delete` (not add/remove), `SmartBlock.set`, `OTP.EnforcePolicy.set`. **Self-lockout guard** (off `CurrentConnection`) proven live: refuses blocking the active source/subnet, refuses enforced-2FA without override. Remaining capability-only: **DoS** read/write param undiscoverable (114). |
| [WI-068](work-items/WI-068-security-advisor.md) | P2 | `done` | C | WI-006 | Security Advisor: **read + guarded-write slices shipped + live-verified on DSM 7.3**. Writes = `Conf.set` (`Input`-wrapped `{argGroup(=baseline home/company), enableSchedule, weekday, hour, minute}`) + `Operation.start {items:"ALL"}` run-scan (array crashes the CGI); loosening the baseline / disabling the schedule flagged HIGH; all reverted in verify. No item-level findings API; custom-checklist baseline out of scope. |
| [WI-069](work-items/WI-069-network.md) | P2 | `in_progress` | C | WI-006 | Network interfaces/bonding/routing: **read + guarded-write slices shipped + live-verified on DSM 7.3** (constrained). Reads: general `SYNO.Core.Network`; interfaces = `.Ethernet` not `.Interface`; routes = `.Router.Static.Route` (4302=unconfigured); `.Proxy` password never decoded. Writes: general `Network.set` live-confirmed; **`Ethernet.set` WIRE-UNVERIFIED** (4302 — DSM drives interface change via a precondition/confirm flow in unfetchable webpack JS; apply refuses, no guessing); bond/route/traffic-control capability-only. **never-sever-the-management-NIC guard** (identifies mgmt NIC by connection-host==IP, fail-closed) proven both directions plan-only; mgmt NIC + gateway never actually changed on the shared lab. |
| [WI-070](work-items/WI-070-loginportal.md) | P2 | `done` | C | WI-006 | Login Portal: **read + guarded-write slices shipped + live-verified on DSM 7.3**. Writes = `Web.DSM.set` (ports/HSTS/redirect/domain), `AppPortal.set`, `ReverseProxy` `create`/`delete uuids[]`; reverse-proxy per-rule shape now live-confirmed (nested frontend/backend, numeric protocol enum, uppercase `UUID`), header secrets via credential_ref. **Never-break-current-session guard** proven plan-only (refuses port move / disable-HTTPS / HSTS). DSM-access writes NO-OP-verified only — no real port/HSTS change on the shared lab (constrained verify). |
| [WI-071](work-items/WI-071-terminalsnmp.md) | P2 | `in_progress` | C | WI-006 | Terminal + SNMP: **read + guarded-write slices shipped + live-verified on DSM 7.3**. Writes = `Terminal.set` (SSH/Telnet/port/console; enabling SSH/Telnet flagged HIGH) + `SNMP.set` v1/v2c (community via `credential_ref`, resolved only at apply, out of plan/hash/log — request-capture tested). Live-verified with full revert (telnet/SNMP left off). Remaining capability-only/WIRE-UNVERIFIED: **enabling SNMPv3** (v3 auth/priv password fields return 2202 — only disable supported) and the **SNMP trap target** (no field even when enabled). |
...
| [WI-073](work-items/WI-073-taskscheduler.md) | P2 | `in_progress` | C | WI-006 | Task Scheduler: **Slice A read shipped + live-verified** (scheduled + triggered task inventory). Slice B (guarded create/edit/delete/run of scheduled+triggered scripts and service tasks) deferred — **HIGH risk (scheduled root code execution)**. |
| [WI-074](work-items/WI-074-dsmupdate.md) | P2 | `in_progress` | C | WI-006 | DSM Update & Restore: **Slice A read shipped + live-verified on DSM 7.3** — installed version + local update status (`SYNO.Core.Upgrade` `status`), update-server offered-update check (`SYNO.Core.Upgrade.Server` `check`, unreachable=unknown), auto-update policy (`SYNO.Core.Upgrade.Setting` v2 `get`), and config-backup status/history (`SYNO.Backup.Config.AutoBackup` `get`/`list`; destination `pwd` never decoded). Independent update/config-backup boundaries; CLI `dsm-update` + 5 `get_dsm_update_*` MCP reads. Offered-version/restart/criticality fields surfaced generically under `details` (no update pending on lab to type them). Deferred (HIGH risk): auto-update-policy + config-backup-schedule guarded writes (Slice B), and DSM install (reboot) + config restore — no install/restore path on any surface. |
| [WI-075](work-items/WI-075-hardware.md) | P3 | `in_progress` | C | WI-006 | Hardware/Power/UPS: **Slice A read shipped + live-verified** (beep `SYNO.Core.Hardware.BeepControl`, fan `…FanSpeed`, LED `…Led.Brightness`, power schedule `…PowerSchedule.load`, power recovery `…PowerRecovery`, UPS `SYNO.Core.ExternalDevice.UPS`; all v1; four independent fail-closed boundaries; UPS SNMP community suppressed on read; no-UPS-attached path graceful). Slice B (guarded beep/fan/LED comfort + high-risk power schedule/recovery/UPS) deferred. |
| [WI-076](work-items/WI-076-externaldevice.md) | P3 | `in_progress` | C | WI-006 | External devices: **Slice A read shipped + live-verified** (USB/eSATA storage `SYNO.Core.ExternalDevice.Storage.USB`/`.eSATA` `list`, printers `…Printer` `list` + Bonjour/AirPrint toggle; UPS is separate — WI-075). Lab had no device attached → item shapes decoded tolerantly (wire-unverified). Slice B (guarded eject/printer settings/spooler) deferred. |
| [WI-077](work-items/WI-077-disk-smart-health.md) | P2 | `in_progress` | C | WI-006 | Disk SMART + health: **Slice A read shipped + live-verified** (per-disk health/lifespan/self-test via `SYNO.Core.Storage.Disk.list`; full SMART attribute table + summary via `SYNO.Storage.CGI.Smart.get_health_info`; self-test log via `get_smart_test_log`; global thresholds via `SYNO.Storage.CGI.HddMan.get`); graceful per-disk "no SMART data" (code 117). Slice B (guarded SMART-test run `do_smart_test` + schedule `SYNO.Storage.CGI.Smart.Scheduler`) deferred. Complements the storage inventory (which lacks per-disk SMART). |
| [WI-078](work-items/WI-078-directory.md) | P2 | `in_progress` | C | WI-006 | Directory services: **Slice A read shipped + live-verified** (AD domain `SYNO.Core.Directory.Domain` join status + LDAP `…LDAP` bind status + synced users/groups via `SYNO.Core.User`/`Core.Group` `type=domain\|ldap`; bind/join password never decoded, no-leak canary; lab unjoined → graceful mode=none). Slice B (guarded AD/LDAP join/leave, bind password via credential_ref) deferred — **HIGH risk (changes NAS auth)**. |
| [WI-079](work-items/WI-079-kmip.md) | P3 | `in_progress` | C | WI-006 | KMIP key management: read client/server config+status (Slice A shipped, `SYNO.Storage.CGI.KMIP` v1); guarded config write deferred (material via credential_ref). High risk (can make encrypted shares unmountable). Coordinates with WI-008. |
| [WI-080](work-items/WI-080-universalsearch.md) | P3 | `in_progress` | C | WI-019, WI-022 | Universal Search index management (package-gated on `SynoFinder`): **Slice A read shipped + live-verified** (indexed-folder list via `SYNO.Finder.FileIndexing.Folder.list`; overall index status via `SYNO.Finder.FileIndexing.Status.get`; both entry.cgi v1). Slice B (guarded add/remove/reindex via hash-bound plan/apply) deferred. |
| [WI-081](work-items/WI-081-automatic-tls-trust-enrollment.md) | P2 | `done` | G | WI-032, WI-048 | Automatic TLS trust enrollment: shared `internal/tlstrust` package + CLI `auth` and Gateway-admin trust-on-first-use enrollment, replacing manual fingerprint paste without weakening pin-on-first-use (a changed pin is never silently re-trusted). Merged as `a468b9a`; focused trust/runtime/CLI/Gateway tests pass. |
...
| [WI-083](work-items/WI-083-fresh-nas-provision.md) | P2 | `in_progress` | H | WI-084, WI-023, WI-086 | Fresh-NAS first-admin provision (CLI + Admin UI + remote MCP): `SYNO.Entry.Request` compound on the auto-logged-in setup session, generated password stored via WI-084 (never returned over MCP). **Decisions (2026-07-21): scope includes online OS install (→WI-085) and first-admin; surface is local admin + remote MCP (→WI-086); reuse not rewrite.** **Shipped this session: shared `Service.ProvisionFirstAdmin`/`FinishSetup` operation (reusing the ported `internal/provision` core), thin `dsmctl provision` CLI (user live-confirmed), and the gateway Admin-UI adapter (`POST …/provision` + vault sink + "Provision fresh NAS" dialog, 5 langs) so a provisioned password lands in the vault and is revealable via WI-084.** Supersedes the earlier `WI-083-nas-provision.md` sketch (nas-setup-workflow branch). Pending: remote MCP tool (WI-086), online install (WI-085), one live Admin-UI fresh-NAS verify. |
...
| [WI-085](work-items/WI-085-dsm-os-install-pat.md) | P3 | `in_progress` | H | WI-023 | DSM OS installation on never-installed hardware, **online-first**. **Shipped + live-verified 2026-07-21**: reverse-engineered the Web Assistant `/webman/*.cgi` API ([[dsm-web-assistant-install-api]]) — `get_state.cgi` detection (not_install/sys_crash/sys_migrat + internet_*_ok + version), `install.cgi` online install, `get_install_progress.cgi`, completion via SYNO.API.Info at the setup URL — as `internal/provision/install.go` + CLI `dsmctl install` (detect-only default; `--install --yes` gated by serial confirm). Live: online-reinstalled DS918+ 192.0.2.255 → DSM 7.3.1-86003 → provisioned admin testuser (password in Windows vault); offline `.pat` verified on 192.0.2.51 → DSM 7.3.2-86009. **End-to-end bring-up consolidated into one command: `dsmctl install --admin-user <u> --create-volume` chains install → first-admin (+ built-in-admin disable + wizard) → all-disk volume (default btrfs RAID5, `--raid`/`--filesystem`/`--allow-unsupported-disks`), reusing the storage plan/apply operations.** Pending: sys_migrat, gateway/MCP surface parity check, full one-shot live re-verify (individual stages live-verified). |
| [WI-086](work-items/WI-086-remote-provision-authorization.md) | P2 | `in_progress` | F | WI-016, WI-045 | Remote-provision authorization: a distinct `nas.provision` scope + three MCP tools over shared operations — `provision_nas` (existing credential-less profile), `provision_discovered_nas` (un-enrolled device by LAN url: private-IP-only guard, TOFU cert pin, creates the profile, never auto-allowlists), and `install_discovered_nas` (online DSM install trigger, LAN-only). All refuse re-provisioning, never return the password, and are stripped from the read-only gateway; managed gateway installs a vault sink. **Shipped this session, unit-verified (incl. TOFU against a loopback TLS server); live remote-client + fresh-NAS verify pending.** LAN-TOFU MITM is a documented accepted bootstrap risk. Amends `gateway-deployment.md`. |
| [WI-087](work-items/WI-087-hyper-backup.md) | P2 | `in_progress` | C | WI-019, WI-022 | Hyper Backup module (package-gated on HyperBackup 4.x, vault view gated on HyperBackupVault): reads tasks + task detail (status/progress/target-online), versions, logs, and the Vault service view; guarded plan/apply run-backup-now and cancel. Task create/edit/delete, restores, and Vault writes deferred. Pulls data protection forward from the gap-inventory "later" list. |
| [WI-088](work-items/WI-088-gateway-nas-credential-copy.md) | P2 | `done` | G | WI-015, WI-032, WI-047 | Gateway Admin NAS list credential copy: audited `POST …/credentials/reveal` returns the vault-stored DSM account+password to the signed-in administrator browser session only (404 without a stored password; env fallback and web-login session material never revealed; nothing over `/mcp`, per WI-008), plus localized Copy account / Copy password row actions. Note: the Admin-UI reveal flow was subsequently extended by WI-084 (rate-limited, password-re-authenticated reveal + Open-NAS/Copy-account). WI-085–087 are consumed by parallel provision/Hyper Backup sessions. |
| [WI-089](work-items/WI-089-snapshot-replication.md) | P2 | `done` | C | WI-019, WI-022, WI-029 | Snapshot Replication module, **shipped + live-verified on DSM 7.3-81168**: per-share btrfs snapshot reads (attributes/config/retention/log/node — all core APIs, no package needed) + guarded snapshot create/edit/delete and share-config writes via hash-bound plan/apply (full lifecycle verified on a `dsmctl-e2e-snap-*` share); replication-plan read package-gated and fail-closed. Found + documented the DSM 7.3-81168 feed deadlock that makes the package uninstallable on that build (needs DSM ≥ 7.3-81179). Retention/schedule writes, replication mutations, restore/rollback, LUN snapshots deferred. WI-084–086 reserved by the provisioning program. |
| [WI-090](work-items/WI-090-snapshot-replication-mutation.md) | P2 | `in_progress` | C | WI-089 | Guarded Snapshot Replication **relation create** source→dest (temp_create → check_remote_conn → DR.Plan create v3 → poll) via hash-bound plan/apply; dest credential resolved from its vault profile **at apply only** (never plan/hash/logs/MCP/model — no-leak). **Headless pairing shipped + live-verified nas51→nas255**: `temp_create` with `auth:"account"` mints the DR credential from the destination vault password (the earlier err-528 `synocredential`-broker block was the wrong auth mode). CLI `--prompt-dest-credential` one-shot fallback; MCP fails closed with a console deep-link. Never-overwrite + health + reachability guards, both-revision binding, async poll-then-confirm; adversarially reviewed. **Credential-model extension (2026-07-21):** per-NAS password *book* (multi-account) + `role: managed|target` (a target is a destination you hold credentials for but do not manage — gated by `Manager.Client` refusal + `DestinationClient` bypass), live-verified target-only dest nas51→nas255. Retention/schedule writes, restore/rollback, LUN snapshots, failover still deferred. |
| [WI-091](work-items/WI-091-dsm-delegated-gateway-login.md) | P0 | `in_progress` | G | WI-032 | Let SPK deployments use current DSM administrator Web Login with an optional local fallback account, while generic containers retain mandatory local-admin setup. |
| [WI-092](work-items/WI-092-gateway-stored-password-connect.md) | P1 | `in_progress` | G | WI-032 | Persist password-enrollment sessions and expose an explicit server-side Connect with saved password action on NAS/password pages without returning the secret. |
...
| [WI-097](work-items/WI-097-gateway-token-least-privilege-defaults.md) | P1 | `proposed` | G | — | MCP-token least-privilege policy (design-review follow-up; requires explicitly superseding or reaffirming the accepted Full-access/365-day/no-expiry design): align the manual UI, OAuth challenge/protected-resource metadata/fallback, per-grant managed-NAS consent, and authoritative server lifetime policy. |
...
| [WI-100](work-items/WI-100-cli-mcp-agent-discoverability.md) | P1 | `done` | E | - | Audit and improve CLI/MCP help so an unfamiliar AI agent can select targets and follow safe read or plan/apply workflows without guessing prerequisites. |
